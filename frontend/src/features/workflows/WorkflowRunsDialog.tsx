    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-3xl max-h-[85vh] flex flex-col p-0 overflow-hidden">
        ...
        <div className="flex flex-1 min-h-0 divide-x divide-border">
          {/* Left: Runs list */}
          <div className="w-64 md:w-72 shrink-0 flex flex-col">
              ...
              {!selectedRun ? (
                ...
                <div className="space-y-4 text-xs">
                  <div className="flex items-center justify-between rounded-lg border border-border bg-card p-3">
                    <div>
                      <p className="font-semibold text-foreground">Run #{selectedRun.id.slice(0, 8)}</p>
                    ...
                    <div className="text-right">
                      ...
                      <p className="text-[11px] font-mono text-muted-foreground mt-0.5">{selectedRun.durationMs} ms</p>
                    </div>
                  </div>

                  {selectedRun.error && (
                    <div className="rounded-lg border border-destructive/30 bg-destructive/10 p-3 text-destructive">
                      <span className="font-semibold">Error Message:</span> {selectedRun.error}
                    </div>
                  )}
                  ...
                  {selectedRun.nodeResults && Object.keys(selectedRun.nodeResults).length > 0 && (
                    <div className="space-y-2 pt-2 border-t border-border">
                      ...
                      <div className="space-y-1.5">
                        {Object.values(selectedRun.nodeResults).map((step) => (
                          <div
                            key={step.nodeId}
                          ...
                          >
                            <div className="flex items-center gap-2">
                              ...
                              <span className="font-medium text-foreground">{step.nodeName || step.nodeId}</span>
                              <span className="text-[10px] text-muted-foreground font-mono">({step.nodeType})</span>
                            </div>
